...
df_variabile_8 <- fillna(df_variabile_8, 0, cols = "visite_totali_corporate")
df_variabile_8 <- fillna(df_variabile_8, 0, cols = "visite_totali_sport")
df_variabile_8 <- fillna(df_variabile_8, 0, cols = "visite_totali_tg24")
df_variabile_8 <- fillna(df_variabile_8, 0, cols = "visite_totali_appwsc")
df_variabile_8 <- fillna(df_variabile_8, 0, cols = "visite_totali_appgtv")
df_variabile_8 <- fillna(df_variabile_8, 0, cols = "visite_totali_appsport")
df_variabile_8 <- fillna(df_variabile_8, 0, cols = "visite_totali_appxfactor") 
...
totale_visite <- sql('select DISTINCT *, (visite_totali_corporate + visite_totali_sport + visite_totali_tg24 + visite_totali_appwsc + visite_totali_appgtv + visite_totali_appsport + visite_totali_appxfactor) as totale_visite 
                     from df_variabile_8')

createOrReplaceTempView(totale_visite,'totale_visite')

df_variabile_9 <- sql('SELECT *,
                      round(count_visits_mobile/(count_visits_mobile + count_visits_desktop + count_visits_other),2) AS percentuale_mobile,
                      round(count_visits_desktop/(count_visits_mobile + count_visits_desktop + count_visits_other),2) AS percentuale_desktop,
                      round(count_visits_other/(count_visits_mobile + count_visits_desktop + count_visits_other),2) AS percentuale_other
                      FROM totale_visite')
...
write.parquet(join7_orari, path_df_variabile_23, mode = "overwrite")
...
df_corretto <- filter(df_variabile_23, isNotNull(df_variabile_23$ext_id))

df_corretto_1 <- filter(df_corretto, df_corretto$numero_reportsuite!=0)

write.parquet(df_corretto_1, path_df_corretto)
