...
scarico <- read.parquet(path_scarico_skyitdev_senza_diz) # leggo lo scarico skyitdev (corporate, tg24, sport)

#scarico <- read.parquet(path_scarico_skyitdev) ## SE E'ANDATO IL DIZIONARIO: leggo lo scarico skyitdev (corporate, tg24, sport)
...
appwsc <- read.parquet(path_scarico_skyappwsc) # leggo scarico app wsc
...
write.parquet(utilizzatori7, path_df_variabile_2)
...
df_variabile_5 <- sql('SELECT DISTINCT DF.*, DEV.count_visits_mobile, DEV.count_visits_desktop, DEV.count_visits_other, DEV.tipo_device
...
df_variabile_5$count_visits_mobile <- ifelse(isNull(df_variabile_5$count_visits_mobile), 0, df_variabile_5$count_visits_mobile)
df_variabile_5$count_visits_desktop <- ifelse(isNull(df_variabile_5$count_visits_desktop), 0, df_variabile_5$count_visits_desktop)
df_variabile_5$count_visits_other <- ifelse(isNull(df_variabile_5$count_visits_other), 0, df_variabile_5$count_visits_other)
#View(head(df_variabile_5,100))

df_variabile_5 <- fillna(df_variabile_5, 0, cols = "visite_totali_appwsc")
df_variabile_5 <- fillna(df_variabile_5, 0, cols = "visite_totali_appgtv")
df_variabile_5 <- fillna(df_variabile_5, 0, cols = "visite_totali_appsport")
df_variabile_5 <- fillna(df_variabile_5, 0, cols = "visite_totali_appxfactor")

df_variabile_5 = withColumnRenamed(df_variabile_5, 'count_visits_mobile', 'count_visits_mobile_skyitdev')

df_variabile_5 = withColumn(df_variabile_5, 'count_visits_mobile', (df_variabile_5$count_visits_mobile_skyitdev + df_variabile_5$visite_totali_appwsc + df_variabile_5$visite_totali_appgtv + df_variabile_5$visite_totali_appsport + df_variabile_5$visite_totali_appxfactor))

df_variabile_5 = drop(df_variabile_5, 'count_visits_mobile_skyitdev')
...
social3 <- sql('SELECT DISTINCT external_id_post_evar,
...
df_variabile_6 <- sql('SELECT DISTINCT DF.*, S.count_visits_from_social, S.social_network
                      FROM df_variabile_5 DF
                      LEFT JOIN df_social S
                      ON DF.ext_id = S.external_id_post_evar')

df_variabile_6 <- fillna(df_variabile_6, 0, cols = "count_visits_from_social")
df_variabile_6$social_network <- ifelse(isNull(df_variabile_6$social_network), 'no_social', df_variabile_6$social_network)
...
secondi_corporate <- remove_too_short(corporate_new, 5) # applico funzione tempo in pagina per skyitdev
...
secondi_tg24 <- remove_too_short(tg24_new, 5)
...
secondi_sport <- remove_too_short(sport_new, 5) # applico funzione tempo in pagina per skyitdev
...
secondi_appwsc <- remove_too_short(appwsc_1, 5) 
...
secondi_appgtv <- remove_too_short(appgtv_1, 5) 
...
secondi_appsport <- remove_too_short(appsport_1, 5) 
...
secondi_appxfactor <- remove_too_short(appxfactor_1, 5) 
...
join1_secondi <-sql('SELECT DISTINCT DF.*, C.secondi_totali_corporate
...
join2_secondi <-sql('SELECT DISTINCT J.*, TG.secondi_totali_tg24
...
join3_secondi <-sql('SELECT DISTINCT J2.*, S.secondi_totali_sport
...
join4_secondi <-sql('SELECT DISTINCT J3.*, W.secondi_totali_appwsc
...
join5_secondi <-sql('SELECT DISTINCT J4.*, G.secondi_totali_appgtv
...
join6_secondi <-sql('SELECT DISTINCT J5.*, ASP.secondi_totali_appsport
...
join7_secondi <-sql('SELECT DISTINCT J6.*, XF.secondi_totali_appxfactor
