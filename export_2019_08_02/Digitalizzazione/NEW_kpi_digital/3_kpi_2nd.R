#Script per digitalizzazione numero 3

source("connection_R.R")
options(scipen = 10000)

#Filtro temporale:
data_fine = as.Date('2018-12-31')

path_df_variabile_7 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_9_df_variabile_7.parquet'

path_df_variabile_8 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_10_df_variabile_8.parquet'

path_df_variabile_9 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_11_df_variabile_9.parquet'

path_df_variabile_10 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_12_df_variabile_10.parquet'

path_df_variabile_11 <- "/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_13_df_variabile_11.parquet"

path_df_variabile_12 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_14_df_variabile_12.parquet'

path_df_variabile_13 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_15_df_variabile_13.parquet'

path_df_variabile_14 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_16_df_variabile_14.parquet'

path_df_variabile_15 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_17_df_variabile_15.parquet'

path_df_variabile_16 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_18_df_variabile_16.parquet'

path_df_variabile_17 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_19_df_variabile_17.parquet'

path_df_variabile_18 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_20_df_variabile_18.parquet'

path_df_variabile_19 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_21_df_variabile_19.parquet'

path_df_variabile_20 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_22_df_variabile_20.parquet'

path_df_variabile_21 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_23_df_variabile_21.parquet'

path_df_variabile_22 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_24_df_variabile_22.parquet'

path_df_variabile_23 <- '/user/matteo.ballerinipuviani/dig_gen_giu_2018_step_25_df_variabile_23.parquet'
...
totale_visite <- sql('select *, (count_visits_mobile + count_visits_desktop + count_visits_other) as totale_visite 
                     from df_variabile_8')
...
corporate_ric$ricorrenza <- ifelse((corporate_2$count > 2), 2, ifelse((corporate_2$count>=1) & (corporate_2$count<=2), 1, 0)) 
...
join1_ric <- sql('SELECT DF.*, C.ricorrenza_corporate
...
join2_ric <- sql('SELECT J.*, T.ricorrenza_tg24
                 FROM join1_ric J
...
join3_ric <- sql('SELECT J2.*, S.ricorrenza_sport
                 FROM join2_ric J2
...
join4_ric <- sql('SELECT J3.*, W.ricorrenza_appwsc
                 FROM join3_ric J3
...
join5_ric <- sql('SELECT J4.*, G.ricorrenza_appgtv
                 FROM join4_ric J4
...
join6_ric <- sql('SELECT J5.*, ASP.ricorrenza_appsport
                 FROM join5_ric J5
...
join7_ric <- sql('SELECT J6.*, XF.ricorrenza_appxfactor
                 FROM join6_ric J6
...
df_variabile_11 <- sql('SELECT *, (ricorrenza_corporate + ricorrenza_tg24 + ricorrenza_sport + ricorrenza_appgtv + 
...
join1_aggregate <- sql("select DF.*, CAA.visite_corporate_appwsc_appgtv
...
join2_aggregate <- sql("select J.*, ED.visite_editoriale
...
join3_aggregate <- sql("select J2.*, WEB.visite_web
...
join4_aggregate <- sql("select J3.*, APP.visite_app
                       from join3_aggregate J3
                       left join visite_app APP
                       on J3.ext_id = APP.ext_id")

createOrReplaceTempView(join4_aggregate, "join4_aggregate")
...
join1_mesi <- sql("select DF.*, MM.max_mesi_corporate_appwsc_appgtv
...
join2_mesi <- sql("select J.*, MME.max_mesi_editoriale
                  from join1_mesi J
...
join3_mesi <- sql("select J2.*, MMW.max_mesi_web
                  from join2_mesi J2
...
join4_mesi <- sql("select J3.*, MMA.max_mesi_app
                  from join3_mesi J3
                  left join max_mesi_app MMA
                  on J3.ext_id = MMA.ext_id")

createOrReplaceTempView(join4_mesi, "join4_mesi")
...
join1_secondi <- sql("select DF.*, SCAA.secondi_corporate_appwsc_appgtv
...
join2_secondi <- sql("select J.*, SED.secondi_editoriale
...
join3_secondi <- sql("select J2.*, SW.secondi_web
...
join4_secondi <- sql("select J3.*, SA.secondi_app
...
df_variabile_15 <- sql("select DF.*, SU.score_utilizzo
...
percentili_score_utilizzo <- sql("select percentile_approx(score_utilizzo, 0.10) as p1,
                                 percentile_approx(score_utilizzo, 0.20) as p2,
                                 percentile_approx(score_utilizzo, 0.30) as p3,
                                 percentile_approx(score_utilizzo, 0.40) as p4, 
                                 percentile_approx(score_utilizzo, 0.50) as mediana, -- q2 = mediana,
                                 percentile_approx(score_utilizzo, 0.60) as p6,    
                                 percentile_approx(score_utilizzo, 0.70) as p7,
                                 percentile_approx(score_utilizzo, 0.80) as p8,
                                 percentile_approx(score_utilizzo, 0.90) as p9, -- 90esimo percentile
...
join1_settimana <- sql("select DF.*, C.visite_settimana_corporate, C.visite_weekend_corporate
...
join2_settimana <- sql("select J.*, T.visite_settimana_tg24, T.visite_weekend_tg24
...
join3_settimana <- sql("select J2.*, S.visite_settimana_sport, S.visite_weekend_sport
...
join4_settimana <- sql("select J3.*, W.visite_settimana_appwsc, W.visite_weekend_appwsc
...
join5_settimana <- sql("select J4.*, G.visite_settimana_appgtv, G.visite_weekend_appgtv
...
join6_settimana <- sql("select J5.*, ASP.visite_settimana_appsport, ASP.visite_weekend_appsport
...
join7_settimana <- sql("select J6.*, XF.visite_settimana_appxfactor, XF.visite_weekend_appxfactor
...
write.parquet(join7_settimana, path_df_variabile_16, mode = "overwrite")
...
join1_orari <- sql("select DF.*, C.visite_corporate_notte, C.visite_corporate_mattina, C.visite_corporate_pomeriggio, C.visite_corporate_sera
...
join2_orari <- sql("select DF.*, T.visite_tg24_notte, T.visite_tg24_mattina, T.visite_tg24_pomeriggio, T.visite_tg24_sera
...
join3_orari <- sql("select DF.*, S.visite_sport_notte, S.visite_sport_mattina, S.visite_sport_pomeriggio, S.visite_sport_sera
...
join4_orari <- sql("select DF.*, W.visite_appwsc_notte, W.visite_appwsc_mattina, W.visite_appwsc_pomeriggio, W.visite_appwsc_sera
...
join5_orari <- sql("select DF.*, G.visite_appgtv_notte, G.visite_appgtv_mattina, G.visite_appgtv_pomeriggio, G.visite_appgtv_sera
...
join6_orari <- sql("select DF.*, ASP.visite_appsport_notte, ASP.visite_appsport_mattina, ASP.visite_appsport_pomeriggio, ASP.visite_appsport_sera
...
join7_orari <- sql("select DF.*, XF.visite_appxfactor_notte, XF.visite_appxfactor_mattina, XF.visite_appxfactor_pomeriggio, XF.visite_appxfactor_sera
