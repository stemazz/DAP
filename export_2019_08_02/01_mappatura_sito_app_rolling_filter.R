...
## Mappatura di sito corporate e app wsc CON data di filtro ROLLING ##
...
start_kpi_sito <- function(df, clienti){
  ...
  df <- sql("SELECT *, datediff(data_di_filtro, date_time_dt) as diff_gg_view from df")
...
add_kpi_sito <- function(df) {
  ...
  df <- sql("SELECT *,
            ...
            CASE WHEN (terzo_livello_post_prop LIKE 'faidate home') THEN visit_uniq ELSE Null END AS wsc_home,
            CASE WHEN (terzo_livello_post_prop LIKE 'faidate primafila') THEN visit_uniq ELSE Null END AS wsc_primafila,
            ...
            CASE WHEN (secondo_livello_post_prop LIKE 'tecnologia' OR secondo_livello_post_prop LIKE 'come-vedere') THEN visit_uniq ELSE Null END AS tecnologia,
            CASE WHEN secondo_livello_post_prop LIKE 'pacchetti-offerte' THEN visit_uniq ELSE Null END AS pacchetti_offerte,
            
            CASE WHEN secondo_livello_post_prop LIKE 'landing' THEN visit_uniq ELSE Null END AS visit_landing, 
            CASE WHEN secondo_livello_post_prop LIKE 'homepage sky' THEN visit_uniq ELSE Null END AS visit_homepage,
            CASE WHEN secondo_livello_post_prop LIKE 'shop' THEN visit_uniq ELSE Null END AS visit_shop_soundbox,
            CASE WHEN secondo_livello_post_prop LIKE 'callmenow' THEN visit_uniq ELSE Null END AS visit_callmenow,
            CASE WHEN (secondo_livello_post_prop LIKE 'acquista' OR secondo_livello_post_prop LIKE 'aol') THEN visit_uniq ELSE Null END AS visit_aol,
            
            CASE WHEN terzo_livello_post_prop LIKE 'info disdetta' THEN visit_uniq ELSE Null END AS visit_InfoDisdetta, -- !!ATTENZIONE verificare post DDL 
            -- CASE WHEN terzo_livello_post_prop LIKE 'trova_skyservice' THEN visit_uniq ELSE Null END AS visit_TrovaSkyService,
            ...
            terzo_livello_post_prop LIKE 'trova_skyservice' OR
            ...
            CASE WHEN (terzo_livello_post_prop LIKE 'faidate home') THEN visit_uniq ELSE Null END AS sec_wsc_home,
            CASE WHEN (terzo_livello_post_prop LIKE 'faidate primafila') THEN visit_uniq ELSE Null END AS sec_wsc_primafila,
            ...
            CASE WHEN (secondo_livello_post_prop LIKE 'tecnologia' OR secondo_livello_post_prop LIKE 'come-vedere') THEN sec_on_page ELSE Null END AS sec_tecnologia,
            CASE WHEN secondo_livello_post_prop LIKE 'pacchetti-offerte' THEN sec_on_page ELSE Null END AS sec_pacchetti_offerte,
            
            CASE WHEN secondo_livello_post_prop LIKE 'landing' THEN visit_uniq ELSE Null END AS sec_landing, 
            CASE WHEN secondo_livello_post_prop LIKE 'homepage sky' THEN visit_uniq ELSE Null END AS sec_homepage,
            CASE WHEN secondo_livello_post_prop LIKE 'shop' THEN visit_uniq ELSE Null END AS sec_shop_soundbox,
            CASE WHEN secondo_livello_post_prop LIKE 'callmenow' THEN visit_uniq ELSE Null END AS sec_callmenow,
            CASE WHEN (secondo_livello_post_prop LIKE 'acquista' OR secondo_livello_post_prop LIKE 'aol') THEN visit_uniq ELSE Null END AS sec_aol,
            
            CASE WHEN terzo_livello_post_prop LIKE 'info disdetta' THEN sec_on_page ELSE Null END AS sec_visit_InfoDisdetta, -- !!ATTENZIONE verificare post DDL 
            -- CASE WHEN terzo_livello_post_prop LIKE 'trova_skyservice' THEN sec_on_page ELSE Null END AS sec_visit_TrovaSkyService,
...
group_kpi_sito <- function(df) {
  ...
  df_kpi <- agg(
    ...
    visite_FaiDaTe_Extra = countDistinct(df$wsc_extra),
    ...
    visite_FaiDaTe_Home = countDistinct(df$sec_wsc_home),
    visite_FaiDaTe_Primafila = countDistinct(df$sec_wsc_primafila),
    ...
    visite_Landing = countDistinct(df$visit_landing),
    visite_Homepage = countDistinct(df$visit_homepage),
    visite_Shop_Soundbox = countDistinct(df$visit_shop_soundbox),
    visite_Callmenow = countDistinct(df$visit_callmenow),
    visite_AOL = countDistinct(df$visit_aol),
    ...
    secondi_FaiDaTe_Extra = sum(df$sec_wsc_extra, na.rm = T),
    ...
    secondi_FaiDaTe_Home = sum(df$wsc_home, na.rm = T),
    secondi_FaiDaTe_Primafila = sum(df$wsc_primafila, na.rm = T),
    ...
    secondi_Landing = sum(df$sec_landing, na.rm = T),
    secondi_Homepage = sum(df$sec_homepage, na.rm = T),
    secondi_Shop_Soundbox = sum(df$sec_shop_soundbox, na.rm = T),
    secondi_Callmenow = sum(df$sec_callmenow, na.rm = T),
    secondi_AOL = sum(df$sec_aol, na.rm = T),
...
add_kpi_app <- function(df) {
  ...
  df <- sql("SELECT *,
            ...
            -- Visite per categoria
            CASE WHEN site_section LIKE '%fattur%' THEN visit_num ELSE Null END AS app_dati_fatture,
            ...
            CASE WHEN (site_section LIKE '%extra%' OR page_name_post_prop LIKE '%extra%') THEN visit_num ELSE Null END AS app_extra,
            ...
            CASE WHEN (site_section LIKE '%home%' OR page_name_post_prop LIKE 'home') THEN visit_num ELSE Null END AS app_home,
            -- CASE WHEN site_section LIKE 'sky service' THEN visit_num ELSE Null END AS app_sky_service,
            CASE WHEN site_section LIKE 'login' THEN visit_num ELSE Null END AS app_login,
            CASE WHEN site_section LIKE 'intro' THEN visit_num ELSE Null END AS app_intro,
            CASE WHEN site_section LIKE '%primafila%' THEN visit_num ELSE Null END AS app_primafila,
            ...
            -- Secondi per categoria
            CASE WHEN site_section LIKE '%fattur%' THEN sec_on_page ELSE Null END AS sec_app_dati_fatture,
            ...
            CASE WHEN (site_section LIKE '%extra%' OR page_name_post_prop LIKE '%extra%') THEN sec_on_page ELSE Null END AS sec_app_extra,
            ...
            CASE WHEN (site_section LIKE '%home%' OR page_name_post_prop LIKE 'home') THEN sec_on_page ELSE Null END AS sec_app_home,
            -- CASE WHEN site_section LIKE 'sky service' THEN sec_on_page ELSE Null END AS sec_app_sky_service,
            CASE WHEN site_section LIKE 'login' THEN sec_on_page ELSE Null END AS sec_app_login,
            CASE WHEN site_section LIKE 'intro' THEN sec_on_page ELSE Null END AS sec_app_intro,
            CASE WHEN site_section LIKE '%primafila%' THEN sec_on_page ELSE Null END AS sec_app_primafila,
            CASE WHEN site_section LIKE 'info' THEN sec_on_page ELSE Null END AS sec_app_info,
            CASE WHEN page_name_post_evar LIKE 'arricchisci abbonamento:lista voucher' THEN sec_on_page ELSE Null END AS sec_Info_DAZN
...
group_kpi_app <- function(df) {
  ...
  df_kpi <- agg(
    ...
    visite_login = countDistinct(df$app_login),
    visite_prima_fila = countDistinct(df$app_primafila),
    ...
    secondi_login = sum(df$sec_app_login, na.rm = T),
    secondi_prima_fila = sum(df$sec_app_primafila, na.rm = T),
    secondi_info = sum(df$sec_app_info, na.rm = T),
    secondi_Info_DAZN = sum(df$sec_Info_DAZN, na.rm = T)
  )
