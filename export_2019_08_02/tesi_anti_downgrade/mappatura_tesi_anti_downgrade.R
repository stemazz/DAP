...
add_kpi_sito <- function(df) {
  ...
  df <- sql("SELECT *,
            ...
            CASE WHEN secondo_livello_post_prop LIKE 'faidate' THEN visit_uniq ELSE Null END AS wsc_tot,
            ...
            CASE WHEN page_url_post_evar LIKE 'http://www.sky.it/assistenza/conosci/informazioni-sull-abbonamento/modificare-l-abbonamento/cosa-devo-fare-per-ridurre-la-composizione-del-mio-abbonamento.html' 
                    THEN visit_uniq ELSE Null END AS visit_Assistenza_Informativa_DG,
            CASE WHEN page_url_post_evar LIKE 'https://www.sky.it/assistenza/contatta.html?first=Gestire-l%27abbonamento&second=togli-un-pacchetto-o-servizio' 
                    THEN visit_uniq ELSE Null END AS visit_Assistenza_Contatta_DG,
            ...
            CASE WHEN page_url_post_evar LIKE 'http://www.sky.it/assistenza/conosci/informazioni-sull-abbonamento/modificare-l-abbonamento/cosa-devo-fare-per-ridurre-la-composizione-del-mio-abbonamento.html' 
                    THEN sec_on_page ELSE Null END AS sec_Assistenza_Informativa_DG,
            CASE WHEN page_url_post_evar LIKE 'https://www.sky.it/assistenza/contatta.html?first=Gestire-l%27abbonamento&second=togli-un-pacchetto-o-servizio' 
                    THEN sec_on_page ELSE Null END AS sec_Assistenza_Contatta_DG
...
group_kpi_sito <- function(df) {
  ...
  df_kpi <- agg(
    ...
    visite_assistenza_informativa_DG = countDistinct(df$visit_Assistenza_Informativa_DG),
    visite_assistenza_contatta_DG = countDistinct(df$visit_Assistenza_Contatta_DG),
    ...
    secondi_assistenza_informativa_DG = sum(df$sec_Assistenza_Informativa_DG),
    secondi_assistenza_contatta_DG = sum(df$sec_Assistenza_Contatta_DG)
  )
