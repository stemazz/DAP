...
path_mappatura_cookie_skyid <- "/user/stefano.mazzucca/anti_downgrade/dizionario.parquet"
path_navigazione_sito <- "/user/stefano.mazzucca/anti_downgrade/nav_sito.parquet"
path_navigazione_app <- "/user/stefano.mazzucca/anti_downgrade/nav_app.parquet"

path_kpi_sito <- "/user/stefano.mazzucca/anti_downgrade/kpi_sito.parquet"
path_kpi_app <- "/user/stefano.mazzucca/anti_downgrade/kpi_app.parquet"
path_kpi_finale <- "/user/stefano.mazzucca/anti_downgrade/kpi_finale.parquet"
...
## Prendo tutta la report suite Adobe corporate
## Navigazioni SITO
...
nrow(diz)
# 5.775.944
extid_cookie <- filter(diz, isNotNull(diz$post_visid_concatenated))
View(head(extid_cookie,100))
nrow(extid_cookie)
# 5.627.470
...
## join tra full e navigazioni (skyit_4)
...
## join tra utenti target e navigazioni (skyappwsc_4)
createOrReplaceTempView(base_1, "base_1")
...
## mappatura sezioni sito/app
source("tesi_anti_downgrade/mappatura_tesi_anti_downgrade.R")
...
query <- ("SELECT COD_CLIENTE_CIFRATO,
          MAX(data_di_filtro) AS data_di_filtro
          FROM clienti 
...
cookie_sito <- filter(cookie_sito, cookie_sito$date_time_dt >= (date_sub(cookie_sito$ultimo_dwg_dt,90))) 
...
cookie_app <- filter(cookie_app, cookie_app$date_time_dt >= (date_sub(cookie_app$ultimo_dwg_dt,90)))
...
df_kpi_2 <- sql("select distinct t1.data_di_filtro as ultimo_dwg_dt, t2.*
                from clienti t1
                left join df_kpi t2
                on t1.COD_CLIENTE_CIFRATO = t2.COD_CLIENTE_CIFRATO")
...
write.df(repartition( df_kpi, 1), path = "/user/stefano.mazzucca/anti_downgrade/df_kpi_finale_anti_dwg.csv", "csv", sep=";", mode = "overwrite", header=TRUE)
...
adform <- read.parquet("/STAGE/adform/table=Trackingpoint/Trackingpoint.parquet")
View(head(adform,100))
