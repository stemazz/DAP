...
group_kpi_sito <- function(df) {
  ...
  df_kpi <- agg(
    ...
    # visite
    visite_totali = count(df$visit_num),
    visite_Fatture = count(df$visit_Fatture),
    visite_FaiDaTeExtra = count(df$visit_FaiDaTeExtra),
    visite_ArricchisciAbbonamento = count(df$visit_ArricchischiAbbonamento),
    visite_Gestione = count(df$visit_Gestione),
    visite_Contatti = count(df$visit_Contatti),
    visite_Comunicazione = count(df$visit_Comunicazione),
    visite_Assistenza = count(df$visit_Assistenza),
    visite_Trasloco = count(df$visit_Trasloco),
    visite_TrasparenzaTariffaria = count(df$visit_TrasparenzaTariffaria),
    visite_GuidaTv = count(df$visit_GuidaTv),
    visite_PaginaDiServizio = count(df$visit_PaginaDiServizio),
    visite_Extra = count(df$visit_Extra),
    visite_Tecnologia = count(df$visit_Tecnologia),
    visite_PacchettiOfferte = count(df$visit_PacchettiOfferte),
    visite_InfoDisdetta = count(df$visit_InfoDisdetta),
    visite_TrovaSkyService = count(df$visit_TrovaSkyService),
    visite_SearchDisdetta = count(df$visit_SearchDisdetta),
    visite_SkyExpert = count(df$visit_SkyExpert),
    visite_Trigger = count(df$visit_Trigger),
...
group_kpi_app <- function(df) {
  ...
  df_kpi <- agg(
    ...
    visite_fatture = count(df$visit_fatture),
    visite_extra = count(df$visit_extra),
    visite_arricchisci_abbonamento = count(df$visit_arricchisci_abbonamento),
    visite_stato_abbonamento = count(df$visit_stato_abbonamento),
    visite_contatta_sky = count(df$visit_contatta_sky),
    visite_assistenza_e_supporto = count(df$visit_assistenza_e_supporto),
    visite_comunicazioni = count(df$visit_comunicazioni),
    visite_gestisci_servizi = count(df$visit_gestisci_servizi),
    visite_mio_abbonamento = count(df$visit_mio_abbonamento),
    visite_info = count(df$visit_info),
