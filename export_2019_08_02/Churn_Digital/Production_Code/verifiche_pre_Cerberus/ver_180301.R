...
path_cb <- "/user/stefano.mazzucca/churn_digital_ver/1803.csv"
path_lista_output <- "/user/stefano.mazzucca/churn_digital_ver/output_modello_CD_180301.csv"

# path_ver <- "/user/stefano.mazzucca/churn_digital_ver/ver_180301.csv"
path_ver_csv <- "/user/stefano.mazzucca/churn_digital_ver/ver_csv_180301.csv"
path_csv1 <- "/user/stefano.mazzucca/churn_digital_ver/csv_pdisc_180301.csv"
...
pivot_tot <- count(pivot(groupBy(ver, "decile_cdm"), "fascia_4_sdm"))
...
pdisc <- filter(ver, isNotNull(ver$pdisc1))
pivot_pdisc <- count(pivot(groupBy(pdisc, "decile_cdm"), "fascia_4_sdm"))
...
rate <- withColumn(ver, "churn_periodo", ifelse(isNotNull(ver$pdisc1), 1, 0))
rate_cdm <- summarize(groupBy(rate, "decile_cdm", "churn_periodo"), count = count(rate$COD_CLIENTE_CIFRATO))
View(head(rate_cdm,100))
rate_sdm <- summarize(groupBy(rate, "fascia_4_sdm", "churn_periodo"), count = count(rate$COD_CLIENTE_CIFRATO))
View(head(rate_sdm,100))




oggetto1 <- filter(ver, isNotNull(ver$pdisc1))
